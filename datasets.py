    ...
    def __getitem__(self, index: int):
        ...
        for k, v in self.imgs.items():
            if index < len(v):
                ...
                index = index - len(v)
        positive_image_paths = self.imgs[cls][index]

        positive_images = [Image.open(img_path).convert('RGB') for img_path in positive_image_paths]
        if self.transform:
            positive_images = [self.transform(positive_image) for positive_image in positive_images]

        return positive_images[0], positive_images[1], cls, positive_image_paths[0], positive_image_paths[1]
...
class NegativeDataset(Dataset):
    ...
    def __init__(self, root: str, query_cls, query_view_index=0,
                 view_suffix=('A', 'B'), img_size=416, transform=None, split='train',
                 ignore_class='negative'):
        """
        Dataset for files with the structure
        ...
        for i, cls in enumerate(self.classes):
            class_path = os.path.join(root, cls, split)
            imgs = [os.path.join(class_path, f) for f in os.listdir(os.path.join(class_path))
                    if is_file(os.path.join(class_path, f)) and is_image_file(f)]
            if cls == ignore_class:
                self.imgs += imgs
            elif cls != query_cls:
                imgs.sort()
                ...
                for img_tuple in imgs:
                    self.imgs += [img for i, img in enumerate(img_tuple) if i != query_view_index]
