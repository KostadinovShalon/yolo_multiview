    ...
    for data in tqdm(dataloader, desc="Detecting objects and saving images"):
        ...
        for batch_id, (img_path, img_detection) in enumerate(zip(img_paths, detections)):
            ...
            if save_structured and gt is not None:
                if targets.size(0) == 0:
                    save_no_gt(img_path, parser.inferece_dir, filename, classes, parser.img_size, img_detection)
                else:
                    save_detection(img_path, img_detection, gt, parser.inference, filename, classes, parser.img_size)
            else:
...
def detect_multiview(model, dataset, f_matrices, parser, weak_conf_th):
    annotations = None
...
# This is a test
# def detect_oneset_multiview(model, views, f_matrices, parser, image_a_path, image_b_path):
...
#
#         conf_t_dets_a = conf_thresholding(view_outputs[v0], 0.8)
#         conf_t_dets_b = conf_thresholding(view_outputs[v1], 0.8)
#
#         draw_detections(image_a_path, "image_a_1step_0.8.png", conf_t_dets_a, classes, parser.img_size)
#         draw_detections(image_b_path, "image_b_1step_0.8.png", conf_t_dets_b, classes, parser.img_size)
#
